...
public class StringUtil {
	 ...
	 * </pre>
	 */
	public static double zeroConvertString(String src) {
...
/**
	 ...
	 * @see
	 */
	public static String getTimeStamp(String type, int month) {
	...
	public static String getSapDataString(Object obj) {
		String returnStr = "";
	 ...
	 * @see
	 */
	public static Integer getEgovMapTotal(Object totalObj, Object recodeObj) {

		int total = 0;
		if (totalObj != null && recodeObj != null) {
			total = Integer.parseInt(totalObj.toString())
					+ Integer.parseInt(recodeObj.toString());
		}else if (totalObj != null && recodeObj == null) {
			total = Integer.parseInt(totalObj.toString())
					+ Integer.parseInt("0");
		} else {
			total = Integer.parseInt(recodeObj.toString());
		}
		return total;
	}
	 ...
	 * @see
	 */
	public static double getEgovMapRate(Object rateCntTotalObj, Object recodeObj) {

		double rate = 0;

		int rateCntTotal = Integer.parseInt(rateCntTotalObj.toString());
		int recodeTotal = Integer.parseInt(recodeObj.toString());

		rate = (Math
				.round(((double) recodeTotal / (double) rateCntTotal) * 1000) / 10.0);
		return rate;
	}
	 ...
	 * @see
	 */
	public static String getLastDay(String formatStr, int num) throws ParseException {
		...
		int _year = Integer.parseInt(beforeYear);
		int _month = Integer.parseInt(beforeMonth);

		Calendar calendar = Calendar.getInstance();
		calendar.set(_year, (_month - 1), 1); // 월은 0부터 시작
		String lastDay = String.valueOf(calendar
		...
		Date tempDate = dateFormatter.parse(beforeYear + beforeMonth + lastDay);

		SimpleDateFormat dateParser = new SimpleDateFormat(formatStr);
		String returnDate = dateParser.format(tempDate);
	 ...
	 * @see
	 */
	public static String changeRDParam(String paramStr) throws Exception {
		String returnStr = "";

		if(paramStr==null) return "";

		paramStr = paramStr.replaceAll("\r\n", "/rn").replaceAll("\r", "/rn").replaceAll("\n", "/rn");

		String[] paramArr = paramStr.split("/rn");

		for(String p : paramArr){
			if(returnStr.length()>0) returnStr += "/rn";
			returnStr += p.trim();
		}
		return returnStr;
	}
	 ...
	 * 현재 날짜 포멧형태 정보 반환
	 *
	 * @param String fommatStr
	 * @return String date 값
	 * @exception MyException
	 * @see
	 */
	public static String getFommatDateStr(String formmatStr) {
		...
		if(formmatStr!=null) pattern = formmatStr;
	 ...
	 * @see
	 */
	public static String uriPathDeletePageUrl(String uriStr, String contextPath){
		if( uriStr == null) return "";
		uriStr = uriStr.replaceAll(contextPath, "");
		uriStr = uriStr.substring(0, uriStr.lastIndexOf("/")+1);
		return uriStr;
	}
}
