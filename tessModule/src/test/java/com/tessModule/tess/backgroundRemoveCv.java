...
public class backgroundRemoveCv {
...
//	nb_img = remove_background(img, 230)	
	...
	public static void test(String filePath) throws Exception {
	    ...
	    Mat th = new Mat();
        Imgproc.threshold(grayDest, th,200,255, Imgproc.THRESH_BINARY);
...
//        Mat out = new Mat(floodFill|floodFill_inv);  //흑백전환을 위한 매트릭스
    	...
    	//과정 저장
	    try{
		      Imgcodecs.imwrite("C:\\img_test\\backgroudTest\\1.gray.png",grayDest); //1.그레이스케일
		      Imgcodecs.imwrite("C:\\img_test\\backgroudTest\\2.threshold.png",th); //2.모핑 그라디언트
		      Imgcodecs.imwrite("C:\\img_test\\backgroudTest\\3.floodFill.png",floodFill); //2.모핑 그라디언트
		      Imgcodecs.imwrite("C:\\img_test\\backgroudTest\\4.floodFill_inv.png", floodFill_inv); //3. 임계값 흑백전환
	    }catch(Exception e){
